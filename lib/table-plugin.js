  ...
  /* -- END ATTRIBUTES -- */
	...
	*/
	Table.prototype.refresh = function () {
		...
		this.calculateVirtualXY( );
	};

	/* -- METHODS -- */
	/**
	 * Calculates the virtual X / Y of a Cell. meaning, which would be the row 
	 * index and cell index if all the rowspans and colspans would be expanded
	 * to real td.
	 * Stores these X / Y values on the Cell.
	 */
	 Table.prototype.calculateVirtualXY = function( ) {
		 var self = this;
		 function insertDummyAt( arr, index ) {
			 arr.splice( index, 0, { type: 'dummy' } );
		 }
		 function insertDummy( entry, y, x ) {
			 var colspan = entry.colspan;
			 var rowspan = entry.rowspan;
			 for( i=0; i<rowspan; i++ ) {
				 for( j=0; j<colspan; j++ ) {
					 if( i==0 ) {
						 if( j>0 ) { // don't duplicate the element with the col-/rowspan
							 insertDummyAt( matrix[ y+i ], x+j );
						 }
					 } else {
						 insertDummyAt( matrix[ y+i ], x+j );
					 }
		 ...
		 var matrix = [];
		 this.obj.find( 'tr' ).each( function( _, tr ) {
			 var row = [];
			 $( tr ).find( selector ).each( function( _, td ) {
				 var $td = $( td );
				 row.push( {
					 cell: $.data( td, 'aloha-table-cell' ),
		 ...
		 $.each( matrix, function( y, row ) {
			 $.each( row, function( x, td ) {
				 var hasColspanOrRowspan = ( td.colspan > 1 ) || ( td.rowspan > 1 );
	...
	/**
	 * Calculates the maximum number of columns of all rows. this includes the 
	 * colspans as well.
	 *
	 * @return the maximum number of columns
	 */
	Table.prototype.getMaxCols = function( ) {
		var maxCols = 0;
		var selector = "td:not(td." + this.get('classLeftUpperCorner') + "), th";
		this.obj.find( 'tr' ).each( function( _, row ) {
			var numberOfCols = 0;
			$( row ).find( selector ).each( function( _, td ) {
				numberOfCols += parseInt( $( td ).attr( 'colspan' ) ) || 1;
			} );
	...
	*/
	...
	var numColumns = this.getMaxCols( );
	...
	*/
	...
	Table.prototype.deleteRows = function() {
		...
		if (deleteTable) {
			...
			/*
			SiFu: disabled it, because - at least the non-ie part - is broken
			if (jQuery.browser.msie){
	...
	*/
	Table.prototype.selectColumns = function () {
		...
		for( var j = 0; j < this.columnsToSelect.length; j++ ) {
			var virtualX = this.columnsToSelect[ j ] - 1;
			var c=0, cell;
			while( cell = this.cells[ c ] ) {
				if( cell.virtualXY.x == virtualX ) {
					// the rest expects raw table tds. so the variable is kinda ill-named
					toSelect.push( cell.obj[ 0 ] );
				}
				c++;
			}
		}
		TableHelper.selectedCells.push(toSelect);
		// blur all editables within the table
	...
	*/
	Table.prototype.selectRows = function () {
		...
		for (var i = 0; i < this.rowsToSelect.length; i++) {
			...
			// old: var rowCells = jQuery(this.obj.find('tr').get(rowId).cells).toArray();

			// filter cells by virtual-Y index (zero based, so rowId-1)
			var rowCells = [];
			// the code below expects the selection column as well.
			rowCells.push( jQuery( this.obj.find('tr').get(rowId).cells[0] ) );
			var c=0, cell, selectedCellsInCol=[];
			while( cell = this.cells[ c ] ) {
				if( cell.virtualXY.y == ( rowId - 1 ) ) {
					// the rest expects raw table tds. so the variable is kinda ill-named
					rowCells.push( cell.obj[ 0 ] );
				}
				c++;
			}
	...
	******************************/
	...
	Table.Cell = function(originalTd, tableObj) {
		...
		$.data( originalTd, 'aloha-table-cell', this );
	...
	/**
	 * virtual coordinates, which would be the row index and cell index
	 * if all the rowspans and colspans would be expaned to real TDs.
	 */
	Table.Cell.prototype.virtualXY = undefined;
